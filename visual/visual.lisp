...
(defun png-vertical-line (png x-png rgb)
  "Draw a vertical line"
  (let ((png-array (data-array png)))
    (when (and (>= x-png 0) (<= x-png (width png)))
      (loop for y from 0 to (- (height png) 1) do
...
(defun png-generate-white-png (width height transformations &optional func)
  ...
  (let* ((png (make-instance 'png
    ...
    (loop for x from 0 to (- width 1) do
         (png-vertical-line png x (list 255 255 255)))
    ...
    (let* ((xy0 (coord-real-to-viewport (list 0 0) transformations))
      ...
      (when (and (>= y0 0) (< y0 width))
        ...
        (png-horizontal-line png (- height y0) (list 0 0 0))))
    (vis-log 3 "axis done")
	;; draw the function
    (when func
      ...
      (loop for x from 0 to (- width 2) do
           (let* ((x-real (first (coord-viewport-to-real
             ...
             (when (and (>= y 0) (< y height))
               ...
               (setf (aref image (- height y 1) (+ x 1) 0) 255)
               (setf (aref image (- height y 1) (+ x 1) 1) 0)
               (setf (aref image (- height y 1) (+ x 1) 2) 0))))
